(function() {
    ...
    app.controller('TakeOutManageGrid',InitGrid);

    var editCtrl = "TakeOutManageGridEdit";
    app.controller(editCtrl,InitEdit);

    var base = '/food/foodinfo/';
    var editUrl = '../nsAdmin/restaurantbooking/takeout/panel_edit.html';

    function InitGrid($scope, $http, $state, $stateParams,HttpUtils,ModalCtrl,modalCode,$modal){

        var currKeys = undefined;

        $scope.url = base + 'takeOutList';
    ...
    function InitEdit($scope,site,$filter,modalCode,ModalCtrl,HttpUtils,$modalInstance) {
        ...
        $scope.addSave = function () {
            if(site.code == 1){
                ...
                sendData.ext1= ext1;
                HttpUtils.post(base + 'addTakeOutSave',sendData,function (data) {
                    site.refresh();
