...
(function () {
    ...
    app.factory('userParkChoose',function ($modal) {
    	var baseUrl = '/baseinfo/park/list';
        var baseHtmlPath = 'baseinfo/user/utils/parkchoose/park_choose.html';
        var baseBatchHtmlPath = 'baseinfo/user/utils/parkchoose/park_choose2.html';
        return {
        	choose:choose,
            chooses:chooses
        };
       ...
       /* //选择事件
        ...
        $scope.commit = function () {
        	if($scope.checkList.length <= 0){
        		ModalCtrl.show('提示','请选择园区',modalCode.success);
        		return;
        	}
        ...
        $scope.join = function(id){
	    	var sendData = {userId:site.sendData.userId,parkId:id};
        ...
        $scope.leave = function(id){
	    	var sendData = {userId:site.sendData.userId,parkId:id};
			HttpUtils.post('/baseinfo/relation/deleteUserPark',sendData,function (data) {
				getGridData();
				ModalCtrl.show('提示','已退出园区',modalCode.success);
			});
        }
        
        $scope.updateDefault = function(id) {
	    	var sendData = {userId:site.sendData.userId,parkId:id};
			HttpUtils.post('/baseinfo/relation/updateUserDefaultPark',sendData,function (data) {
				getGridData();
