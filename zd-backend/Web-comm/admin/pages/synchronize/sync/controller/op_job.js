(function() {
	 ...
	 app.controller('SyncOpJobGrid',Grid);
	 
	 var editCtrl = "SyncOpJobEdit";
	 app.controller(editCtrl,DCtrl);

	 var base = '/synchronize/opjob/';
	 var editUrl = '../admin/pages/synchronize/sync/op_job_edit.html';
	...
	function DCtrl(site,$filter,modalCode,ModalCtrl,HttpUtils,$modalInstance,$http) {
		...
		mm.form = {};//清空表单数据
		mm.select=[];
		if(!mm.form.status)mm.form.status = 1;
		if(mm.site.code == 1){
