(function() {
	 ...
	 var base = '/authority/init/';
	 //var editUrl = '../admin/pages/baseinfo/role/role_edit.html';
	 var tmplBase = '../admin/pages/baseinfo/parkresource/utils';
...
//	    	}
	 	...
	 	$scope.userlist = function (parkId) {
		 		...
		 		HttpUtils.get(base + "parkAdmins" , sendData ,function (resp) {
	 				...
	 				angular.forEach(list,function(obj,i){
	 					if(obj){
	 						checkedRows[i] = {userId:obj.id};
	 					}
	 				});
		 			
	 				userChoose.chooseUsers(function (users) {
		            	saveRoleUsers(parkId,users);
		            },checkedRows,parkId,{powerKey:"auparkadmin:update"});
      ...
      //查看已有用户
        $scope.managerlist = function (parkId) {
        	var sendData = {
        			parkId:parkId
        	};
        	var params = [];
        	//HttpUtils.get(base + "parkAdmins" , sendData ,function (resp) {
        	//	$scope.managerlistData = resp.data;
        		
        		userChoose.chooseUserView(function (users) {
	            	
	            },params,parkId,{powerKey:"auparkadmin:update"});
	 	...
	 	$scope.resourcelist= function (parkId) {
	 		...
	 		//if(!$scope.isRemote){
	 			...
	 			HttpUtils.get(base + "parkResources" , sendData ,function (resp) {
	 				...
	 				},checkedList,parkId=='platformPark'?false:true,{powerKey:"auparkresource:update"});
