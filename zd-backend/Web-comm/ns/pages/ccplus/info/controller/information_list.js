(function() {
	 ...
	 app.controller('CcInformationGrid',Grid);
	 
	 var editCtrl = "CcInformationEdit";
	 app.controller(editCtrl,DCtrl);

	 var base = '/ccplus/information/';
	 var editUrl = '../admin/pages/ccplus/info/information_edit.html';
	 var categoryUrl = '/basetree/';
	 var displayType = 1;
	...
	function DCtrl(site,$filter,modalCode,ModalCtrl,HttpUtils,$modalInstance,$http) {
		...
		mm.form = {};//清空表单数据
		mm.select=[];
		if(!mm.form.publish)mm.form.publish = -1;
		...
		HttpUtils.get(categoryUrl + 'getParentData', {displayType:displayType}, function (data) {
        	mm.site.categoryList=data.data;
        });
		mm.addSave = function () {
			if(mm.site.code == 1){
				...
				sendData.displayType = displayType;
