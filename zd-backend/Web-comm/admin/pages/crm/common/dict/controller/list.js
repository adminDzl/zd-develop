(function() {
	 ...
	 app.controller('CrmDictGrid',InitGrid);
	 
	 var editCtrl = "CrmDictGridEdit";
	 app.controller(editCtrl,InitEdit);

	 var base = '/crm/common/dict/';
	 var editUrl = '../admin/pages/crm/common/dict/panel_edit.html';
	 ...
	 function open(site,$modal) {
		...
		function InitEdit($scope,site,$filter,modalCode,ModalCtrl,HttpUtils,$modalInstance) {
			...
			if(site.code == 1){
				$scope.form.orderIndex=10;
			}
			if(site.code == 2){
				$scope.form = angular.copy(site.data.entity);
				if($scope.form.orderIndex==null){
					$scope.form.orderIndex=10;
				}
			};
			$scope.parentList = site.data.data_dict;
