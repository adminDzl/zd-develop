...
(function () {
    ...
    app.factory('userDeptChoose',function ($modal) {
    	var baseUrl = '/baseinfo/basDept/grid';
        var baseHtmlPath = 'baseinfo/user/utils/deptchoose/dept_choose.html';
        var baseBatchHtmlPath = 'baseinfo/user/utils/deptchoose/dept_choose2.html';
        return {
        	choose:choose,
            chooses:chooses
        };
        ...
        function chooses(sendback,checkedIds,gridUrl,sendData,gridHtmlPath) {
            ...
            if(!site.gridHtmlPath){
            	site.gridHtmlPath = baseBatchHtmlPath;
            }
            chooseHtml(site);
        };
    ...
    app.controller('DeptChooseCtrl',function (site,$modalInstance,HttpUtils,ModalCtrl,modalCode,$scope) {
        ...
        $scope.commit = function () {
        	if($scope.checkList.length <= 0){
        		ModalCtrl.show('提示','请选择部门',modalCode.success);
        		return;
        	}
        ...
        $scope.join = function(id){
	    	var sendData = {userId:site.sendData.userId,deptId:id};
			HttpUtils.post('/baseinfo/relation/saveUserDept',sendData,function (data) {
				getGridData();
				ModalCtrl.show('提示','加入部门成功',modalCode.success);
			});
        }
        $scope.leave = function(id){
	    	var sendData = {userId:site.sendData.userId,deptId:id};
			HttpUtils.post('/baseinfo/relation/deleteUserDept',sendData,function (data) {
				getGridData();
				ModalCtrl.show('提示','已退出部门',modalCode.success);
			});
        }
        
        $scope.updateDefault = function(id) {
	    	var sendData = {userId:site.sendData.userId,deptId:id};
			HttpUtils.post('/baseinfo/relation/updateUserDefaultDept',sendData,function (data) {
				getGridData();
