(function () {
    ...
    app.controller('TClientGrid', Grid);
    
    var editCtrl = "TClientEdit";
	 app.controller(editCtrl,DCtrl);
	 
	 var base = '/thirdparty/client/';
	 var editUrl = '../admin/pages/thirdparty/client/client_edit.html';
    ...
    function Grid($scope, $http, $state, $stateParams, HttpUtils, ModalCtrl, modalCode, $modal) {
      ...
      //新增
	     ...
	     $scope.downandup = function(row,type){
	    	var sendData = {'id':row.id,'type':type};
 			HttpUtils.post(base +  'downandup',sendData,function (data) {
 					getGridData();
 					ModalCtrl.show('提示','操作成功！',modalCode.success);
 				}
 			);
	     }
